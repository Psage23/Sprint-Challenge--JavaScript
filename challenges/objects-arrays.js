...
const unisWithUni = universities.filter(name => name.includes('Uni'));
...
zooAnimals.forEach(zooAnimals => {
  displayNames.push(`Name: ${zooAnimals.animal_name}, Scientific: ${zooAnimals.last_name}`);
  });
...
const lowCaseAnimalNames = [];
  zooAnimals.map((zooAnimals, index) => {
    lowCaseAnimalNames[index] = zooAnimals.animal_name.toLowerCase();
  });
...
*/
const populationTotal = zooAnimals.reduce((currentValue, zooAnimals) => {
  return currentValue + zooAnimals.population;
}, 0);
